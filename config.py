...
# Suggested follow-up questions settings
SUGGESTED_QUESTIONS_ENABLED = (
    os.getenv("CONTEXTIQ_SUGGESTED_QUESTIONS_ENABLED", "true").lower()
    in ("true", "1", "yes")
)
SUGGESTED_QUESTIONS_COUNT = _env_int("CONTEXTIQ_SUGGESTED_QUESTIONS_COUNT", 4)
SUGGESTED_QUESTIONS_MAX_INPUT_CHARS = _env_int(
    "CONTEXTIQ_SUGGESTED_QUESTIONS_MAX_INPUT_CHARS", 4000
)
SUGGESTED_QUESTIONS_MODEL = os.getenv(
    "CONTEXTIQ_SUGGESTED_QUESTIONS_MODEL", LLM_MODEL_NAME
)
SUGGESTED_QUESTIONS_TIMEOUT = _env_int("CONTEXTIQ_SUGGESTED_QUESTIONS_TIMEOUT", 30)

# Document comparison settings
COMPARISON_MAX_CHUNKS_PER_DOC = _env_int("CONTEXTIQ_COMPARISON_MAX_CHUNKS_PER_DOC", 4)
COMPARISON_MAX_INPUT_CHARS = _env_int("CONTEXTIQ_COMPARISON_MAX_INPUT_CHARS", 6000)
COMPARISON_MODEL_NAME = os.getenv("CONTEXTIQ_COMPARISON_MODEL", LLM_MODEL_NAME)
COMPARISON_TIMEOUT = _env_int("CONTEXTIQ_COMPARISON_TIMEOUT", 60)

# Citation verification settings
CITATION_VERIFICATION_ENABLED = (
    os.getenv("CONTEXTIQ_CITATION_VERIFICATION_ENABLED", "true").lower()
    in ("true", "1", "yes")
)
CITATION_VERIFICATION_MAX_CLAIMS = _env_int(
    "CONTEXTIQ_CITATION_VERIFICATION_MAX_CLAIMS", 8
)
CITATION_VERIFICATION_MAX_EVIDENCE_CHARS = _env_int(
    "CONTEXTIQ_CITATION_VERIFICATION_MAX_EVIDENCE_CHARS", 6000
)
CITATION_VERIFICATION_TIMEOUT = _env_int(
    "CONTEXTIQ_CITATION_VERIFICATION_TIMEOUT", 30
)
CITATION_VERIFICATION_MODEL = os.getenv(
    "CONTEXTIQ_CITATION_VERIFICATION_MODEL", LLM_MODEL_NAME
)

# Contradiction detection settings
CONTRADICTION_DETECTION_ENABLED = (
    os.getenv("CONTEXTIQ_CONTRADICTION_DETECTION_ENABLED", "true").lower()
    in ("true", "1", "yes")
)
CONTRADICTION_MAX_CLAIMS = _env_int(
    "CONTEXTIQ_CONTRADICTION_MAX_CLAIMS", 10
)
CONTRADICTION_MAX_EVIDENCE_CHARS = _env_int(
    "CONTEXTIQ_CONTRADICTION_MAX_EVIDENCE_CHARS", 7000
)
CONTRADICTION_TIMEOUT = _env_int(
    "CONTEXTIQ_CONTRADICTION_TIMEOUT", 30
)
CONTRADICTION_MODEL = os.getenv(
    "CONTEXTIQ_CONTRADICTION_MODEL", LLM_MODEL_NAME
)
